...
const HomeScreen = () => {
  ...
  const handleCategoryClick = (cat) => {
    if (activeCategory === cat) {
      ...
      // Chuyển hướng sang trang chi tiết danh mục đó ở AllProductsScreen
      if (activeGroup) {
          navigate(`/products/${activeGroup}/category/${encodeURIComponent(cat)}`);
      } else {
          // Fallback nếu không có activeGroup (dù điều này hiếm khi xảy ra vì AllProducts bắt buộc có group)
          navigate(`/products/offset/category/${encodeURIComponent(cat)}`);
      }
    }
  };
