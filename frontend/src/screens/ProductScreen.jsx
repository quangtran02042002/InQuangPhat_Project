...
import { 
  ...
  FaHome, FaTag, FaBoxOpen, FaInfoCircle, FaArrowRight, FaPrint 
} from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // (Trong thực tế nên có API riêng: /api/products?category=...)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8">
                        ...
                        <div className="flex gap-3 mt-4 overflow-x-auto pb-2 scrollbar-hide">
                            {product.images.map((img, index) => (
                                <button 
                                    ...
                                    className={`relative w-20 h-20 rounded-lg border-2 overflow-hidden flex-shrink-0 transition-all ${activeImage === img.url ? 'border-blue-600 ring-2 ring-blue-100' : 'border-gray-200 hover:border-gray-400'}`}
                                >
                ...
                {product.priceTable && product.priceTable.length > 0 && (
                    <div className="mb-8 bg-blue-50 rounded-xl p-6 border border-blue-100">
                        <h3 className="text-lg font-bold text-blue-900 mb-4 flex items-center uppercase">
                            <FaPrint className="mr-2" /> Bảng giá tham khảo
                        </h3>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left bg-white rounded-lg overflow-hidden shadow-sm">
                                <thead className="bg-blue-100 text-blue-800 uppercase font-bold text-xs">
                                    <tr>
                                        <th className="px-4 py-3">Số lượng</th>
                                        <th className="px-4 py-3">Đơn giá (VNĐ)</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {product.priceTable.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-gray-50">
                                            <td className="px-4 py-3 font-medium">&ge; {item.minQuantity}</td>
                                            <td className="px-4 py-3 text-red-600 font-bold">{item.price.toLocaleString()} đ</td>
                                            <td className="px-4 py-3 text-gray-500 italic">Giá chưa bao gồm VAT</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        </div>
                        <p className="text-xs text-blue-600 mt-3 italic">* Giá có thể thay đổi tùy thời điểm và yêu cầu gia công thêm.</p>
                    </div>
                )}
            ...
            <div className="lg:col-span-4 space-y-8">
                ...
                <div className="bg-white p-6 rounded-xl border border-blue-200 shadow-md sticky top-24">
                    ...
                    <a href={CONTACT_INFO.zaloLink} className="block w-full text-center bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 transition">
