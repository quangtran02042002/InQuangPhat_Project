...
const PaperPriceScreen = () => {
  ...
  const fetchAllData = async () => {
    try {
      const [papersRes, sizesRes, weightsRes, surchargesRes, suppliersRes] = await Promise.all([
          ...
          axios.get('/api/v1/suppliers', authConfig) // ĐÃ SỬA: Gọi đúng API v1 của bạn
      ]);
      ...
      setSuppliersList(suppliersRes.data.suppliers || suppliersRes.data || []); 
        ...
        toast.error("Lỗi 404 hoặc mất kết nối API. Vui lòng kiểm tra lại Server."); 
    }
  };
  ...
  return (
    <div className="flex h-screen bg-[#F9FAFB] font-sans text-[#111827]">
      ...
      <div className="flex-1 flex flex-col w-full overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-200 px-4 md:px-8 py-3 md:py-4 shrink-0 flex flex-col sm:flex-row justify-between gap-4">
            ...
            <div className="flex items-center gap-3 self-end sm:self-auto">
                <button className="hidden md:flex items-center gap-2 bg-gray-100 text-[#6B7280] hover:text-[#111827] px-4 py-2 rounded-full text-sm font-bold transition">
                    <FaFileExport /> <span className="hidden lg:inline">Xuất file</span>
                </button>
                <button 
        ...
        <div className="sm:hidden bg-white px-4 border-b border-gray-200 flex gap-6 overflow-x-auto text-sm font-medium [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {['Giá giấy gốc', 'Khổ giấy', 'Định lượng', 'Phụ phí'].map(tab => (
                      ...
                      <form id="paperForm" onSubmit={submitHandler} className="space-y-6">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 bg-[#E6F0ED]/40 p-4 md:p-6 rounded-2xl border border-[#006B4D]/10">
                              <div className="sm:col-span-3 lg:col-span-1">
                                  ...
                                  {suppliersList.length === 0 && <p className="text-xs text-red-500 mt-1">* Vui lòng tạo NCC ở trang Danh sách đối tác.</p>}
                              </div>
      ...
      <ConfirmModal isOpen={isDeleteModalOpen} onClose={()=>setIsDeleteModalOpen(false)} onConfirm={deleteHandler} title="Xóa dữ liệu" message="Bạn chắc chắn muốn xóa nhóm giấy này?" />
    </div>
  );
};
